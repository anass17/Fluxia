...
def get_all_users(
    db: Session = Depends(get_db), user_id=Depends(require_roles("ADMIN", "OWNER"))
):
...
def get_all_admins(
    db: Session = Depends(get_db), user_id=Depends(require_roles("OWNER"))
):
    service = UserService(db)
...
def promote_staff(
    id: int, db: Session = Depends(get_db), user_id=Depends(require_roles("OWNER"))
):
    service = UserService(db)
