...
class UserService:
    ...
    def promote_staff(self, id):
        ...
        if not user:
            ...
            raise HTTPException(
                status.HTTP_400_BAD_REQUEST, "You can only promote staff members"
            )
    ...
    def demote_staff(self, id):
        ...
        if not user:
            ...
            raise HTTPException(
                status.HTTP_400_BAD_REQUEST, "You can only demote admins"
            )
    ...
    def block_user(self, current_role, user_id):
        ...
        if not user:
            ...
            raise HTTPException(
                status.HTTP_400_BAD_REQUEST, "Could not block this user"
            )
    ...
    def activate_user(self, current_role, user_id):
        ...
        if not user:
            ...
            raise HTTPException(
                status.HTTP_400_BAD_REQUEST, "Could not activate this user"
            )
        elif current_role == "ADMIN" and user.role == "ADMIN":
            raise HTTPException(
                status.HTTP_400_BAD_REQUEST, "Could not activate this user"
