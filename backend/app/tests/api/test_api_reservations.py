...
def test_get_taken_timeslots(client, db_session):
    ...
    res = Reservation(table_number=1, date=test_date, time="19:00", user_id=99, note="", guests=4)
    db_session.add(res)
    db_session.commit()
...
def test_get_all_reservations_as_staff(as_admin, db_session):
    res1 = Reservation(table_number=1, date=date.today(), time="12:00", user_id=1, note="", guests=4)
    res2 = Reservation(table_number=2, date=date.today(), time="13:00", user_id=2, note="", guests=4)
