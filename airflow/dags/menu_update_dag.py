...
def transform_recipes(ti):
    ...
    data = data[
        data["TotalTime"].str.replace("PT", "").str.replace("M", "").astype(int) <= 15
    ]

    data = data[data["RecipeIngredientParts"].str.find("vodka") < 0]

    data["TotalTime"] = (
        data["TotalTime"].str.replace("PT", "").str.replace("M", "").astype(int)
    )
...
with DAG(
    ...
    extract = PythonOperator(task_id="get_recipes", python_callable=call_api)

    transform = PythonOperator(
        task_id="transform_recipes", python_callable=transform_recipes
    )

    load = PythonOperator(task_id="load_recipes", python_callable=load_recipes)
