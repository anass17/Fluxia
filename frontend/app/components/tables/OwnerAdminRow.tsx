export default function OwnerAdminRow({ staff, fetcher, downgradeFetcher }: { staff: any; fetcher: any, downgradeFetcher: any }) {
    ...
    <tr className="group hover:bg-slate-50/50 transition-colors">
      ...
      <td className="p-4">
        <div className="flex items-center justify-end gap-3">
          ...
          <downgradeFetcher.Form method="put" action={`/owner/user/${staff.id}/downgrade`}>
            <button 
              type="submit"
              disabled={isSubmitting}
              className="text-[10px] uppercase tracking-[0.15em] font-black px-4 py-2 rounded-md border border-rose-100 text-rose-500 hover:bg-rose-500 hover:text-white hover:border-rose-500 transition-all active:scale-95 disabled:opacity-50"
            >
              {isSubmitting ? "..." : "Downgrade"}
            </button>
